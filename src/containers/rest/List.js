...
const RestList = (props) => {
  ...
  const retrieveList = useCallback(
    (filterData = { filter: {} }, isRefresh) => {
      const { filter, ...restFilterData } = filterData;
      const filterObj = {
        outerFilter: {},
        ...props.initialFilter?.filter,
        ...filter,
      }
      const { outerFilter, ...restFilter } = filterObj;
  ...
  useLayoutEffect(() => {
    ...
    retrieveList(!isEmpty(filter) ? filter : { limit: 10, offset: 0, filter: {} }, true);
  ...
  const gotoCreatePage = () => {
    const { redirects, resource, rootPath, initCreateData, customRoute } = props;
  ...
  return (
    <RestListComponent
      ...
      {...props}
      loading={props?.showLoading ? loading : false}
