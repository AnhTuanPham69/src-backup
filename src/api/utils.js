import { get as getData, omit } from 'lodash';
...
function requestWrapper(method) {
  const request = async (url, data = null, params = {}) => {
    let convertUrl = `${process.env.REACT_APP_SERVER_URL}${getData(data, 'isNotAdminUrl') ? '' : '/admin'}${url}`;
    let convertParams = params;
    let convertData = omit(data, ['isNotAdminUrl']);
